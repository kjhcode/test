...
with tabs[2]:
    ...
    with st.expander("Q1. 코딩을 한 번도 안 해봤는데 괜찮을까요?"):
        st.success("네, 전혀 문제없어요! <br> 정보교과는 코딩 경험이 없는 친구들도 기초부터 차근차근 배울 수 있도록 구성되어 있어요. <br>처음에는 조금 낯설 수 있지만, 꾸준히 따라오면 누구나 재미있게 배울 수 있답니다!")
    with st.expander("Q2. 수학을 잘 못하는데 정보교과를 따라갈 수 있을까요?"):
        st.success("정보교과에서 일부 수학적인 개념(예: 알고리즘의 효율성)이 나오기도 하지만, <br>고도의 수학 실력이 필수적인 것은 아니에요. <br>논리적으로 생각하고 문제를 해결하려는 의지가 더 중요하답니다! <br>필요한 수학 개념은 수업 시간에 충분히 설명해 줄 거예요.")
    with st.expander("Q3. 정보교과를 배우면 어떤 진로에 도움이 되나요?"):
        st.success("정보교과는 IT 분야(소프트웨어 개발자, 데이터 과학자, 인공지능 전문가 등)는 물론이고, <br>데이터를 다루거나 디지털 기술을 활용하는 거의 모든 분야의 진로에 도움이 돼요. <br>경영, 의료, 교육, 예술 등 어떤 분야를 선택하든 정보 활용 능력은 큰 강점이 될 거예요!")
    with st.expander("Q4. 정보교과 수업은 딱딱하고 재미없을 것 같아요."):
        st.success("전혀 그렇지 않아요! <br> 정보교과는 이론만 배우는 게 아니라, <br> 직접 코딩하고 결과물을 만들어보면서 성취감을 느낄 수 있는 활동적인 과목이에요. <br>친구들과 함께 프로젝트를 하거나 아이디어를 공유하면서 재미있게 배울 수 있답니다!")

# 네 번째 탭 - 진로 + 퀴즈 + 적성 + 의견
with tabs[3]:
    st.header("🖥️ 정보교과와 연결된 진로/학과")
    st.markdown("""
    💡 <b>컴퓨터/소프트웨어</b>: 개발자, 보안 전문가, 게임 프로그래머 등<br>
    💡 <b>데이터/AI</b>: 데이터 과학자, 데이터 분석가, 인공지능 개발자. 머신러닝 엔지니어, 빅데이터 전문가, AI 연구자 등<br>
    💡 <b>융합기술</b>: 로봇공학, 스마트팜, 디지털 헬스케어, 에듀테크 개발자 등
    """, unsafe_allow_html=True)
