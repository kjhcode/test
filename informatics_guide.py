...
- [2022 개정 교육과정/정보과 - 네이버 블로그][[4]](https://m.blog.naver.com/math_rani/223294591331)
...
#----------------------------------------------------------------------------------------------------------------------------------------
st.subheader("📚 인공지능 기초(진로선택) 과목에서 무엇을 배울까요? (2022 개정 교육과정 기준)") # 헤더 아이콘 유지
st.markdown("""
...
#----------------------------------------------------------------------------------------------------------------------------------------
st.subheader("📚 데이터 과학(진로선택)과목에서 무엇을 배울까요? (2022 개정 교육과정 기준)") # 헤더 아이콘 유지
st.markdown("""
...
info_areas = {
    ...
    " <span style='font-size: 0.7em;'>📈 데이터 준비와 분석</span>": "데이터 분석을 위한 데이터 수집, 전처리 과정. 데이터 분석에 효과적인 형태로 변환", # 아이콘 크기 조절
...
#----------------------------------------------------------------------------------------------------------------------------------------
st.subheader("📚 소프트웨어와 생활(융합선택)과목에서 무엇을 배울까요? (2022 개정 교육과정 기준)") # 헤더 아이콘 유지
st.markdown("""
...
info_areas = {
    " <span style='font-size: 0.7em;'>💻 세상을 변화 시키는 소프트웨어</span>": "소프트웨어를 통해 세상을 변화시킨 사례 탐색.", # 아이콘 크기 조절
...
#----------------------------------------------------------------------------------------------------------------------------------------
st.subheader("📚 정보과학(진로선택)과목에서 무엇을 배울까요? (2022 개정 교육과정 기준)") # 헤더 아이콘 유지
st.markdown("""
이전 교육과정에서는 과학계열 전문교과1에 포함되어 있던 정보과학이 2022개정 교육과정에서는 정보 교과의 진로 선택 과목으로 포함되었습니다. <br>
2022개정 교육과정에서 정보과학의 내용 체계는 자료처리가 데이터 구조로 컴퓨팅 시스템 대신 정보과학 프로젝트로 변경됨.<br>
...
# 진로/학과 데이터 - 아이콘 변경
# 딕셔너리 정의는 그대로 두면 돼! 키에 HTML 문자열이 포함되어 있어.
career_data = {
    "<span style='font-size: 0.7em;'>💻 컴퓨터/소프트웨어 분야": { # 이 문자열 자체가 키가 되는 거야!
...
career_list = list(career_data.items()) # 딕셔너리를 (키, 값) 쌍의 리스트로 변환

# ✨ 첫 번째 반복문 부분은 삭제했어! 컬럼으로만 보여줄 거니까! ✨

for i, col in enumerate(career_cols):
    field, info = career_list[i]
    with col: # 각 컬럼 안에 내용 넣기
        # ✨ st.subheader 대신 st.markdown을 사용해서 HTML을 해석하도록 해! ✨
        # 제목을 좀 더 눈에 띄게 하기 위해 마크다운 헤더(###)와 함께 사용했어!
        st.markdown(f"### {field}", unsafe_allow_html=True)

        st.write(info["설명"])

        # ✨ 딕셔너리 키 이름에 맞게 ' (예시)'를 추가해서 수정했어! ✨
...
st.header("📚 정보교과 적성 간단 테스트") # 헤더 아이콘 유지
