...
st.title("💻 미래를 코딩하다! 정보교과 안내") # 제목 아이콘 유지

st.markdown("""
안녕, 친구들! 😊 <br>
2학년 과목 선택 때문에 고민이 많을 텐데, <br>
혹시 '정보'/'프로그래밍'/인공지능 기초'/'빅데이터 분석' 과목에 대해 들어봤니? <br> <br> 
정보교과는 <br>
단순히 컴퓨터를 배우는 걸 넘어서, <br>
<span style="color: red;">** 우리가 살아갈 미래 사회에서 꼭 필요한 생각하는 힘과 문제 해결 능력을 키워**</span>주는 아주 중요한 과목이야!<br>
왜 정보교과를 선택해야 하는지, <br>
같이 한번 알아볼까?
...
st.write("""
우리가 사는 세상은 빠르게 변하고 있어. <br>
인공지능, 빅데이터, 자율주행... <br>
이 모든 것들이 '정보' 기술과 관련이 깊지! <br>
정보교과를 배우면 이런 변화를 이해하고, 미래 사회의 주인공으로 성장하는 데 필요한
핵심 역량을 기를 수 있어.  <br> [[8]](https://namu.wiki/w/2022%20개정%20교육과정/정보과)
""")

st.header("📚 정보교과에서 무엇을 배울까? (2022 개정 교육과정 기준)") # 헤더 아이콘 유지
st.write("""
2022 개정 교육과정의 정보교과는 크게 5가지 영역으로 나뉘어 있어. [[8]](https://namu.wiki/w/2022%20개정%20교육과정/정보과) <br>
이 영역들을 통해 컴퓨팅 사고력을 기르고 실생활 문제를 해결하는 방법을 배우게 될 거야!
""")

# 정보교과 5가지 영역 - 아이콘 변경
info_areas = {
    "컴퓨팅 시스템 <span style='font-size: 0.9em;'>💻</span>": "컴퓨터가 어떻게 작동하는지, 하드웨어와 소프트웨어의 기본 원리를 이해해.", # 아이콘 크기 조절
    "데이터 <span style='font-size: 0.9em;'>📈</span>": "다양한 데이터를 수집하고 분석해서 의미 있는 정보를 찾아내는 방법을 배워.", # 아이콘 크기 조절
    "알고리즘과 프로그래밍 <span style='font-size: 0.9em;'>🧩</span>": "문제를 해결하기 위한 절차(알고리즘)를 설계하고, 컴퓨터 언어(파이썬 등)로 프로그램을 만드는 연습을 해.", # 아이콘 크기 조절
    "인공지능 <span style='font-size: 0.9em;'>🤖</span>": "스스로 학습하고 판단하는 인공지능의 기본 개념과 활용 사례를 탐구해.", # 아이콘 크기 조절
    "디지털 문화 <span style='font-size: 0.9em;'>🌐</span>": "디지털 세상에서의 올바른 윤리 의식과 시민 역량을 길러." # 아이콘 크기 조절
}
...
우리 학교 정보교과 수업은  <br> 이론과 실습의 균형을 맞춰 진행돼요.  <br> 최신 컴퓨터 실습실에서 파이썬, C++ 등 다양한 프로그래밍 언어를 배우고,  <br> 직접 프로그램을 만들어보는 활동을 많이 해요.

특히, **(학교만의 특별한 점)** 인공지능 윤리 토론 대회나 데이터 분석 프로젝트 발표회 같은 교내 활동과 연계해서 배우는 내용을 더 깊이 탐구할 기회가 많아요.  <br> 정보 선생님께서도 학생들의 눈높이에 맞춰 쉽고 재미있게 가르쳐주셔서 수업 시간이 지루할 틈이 없답니다!
""")
...
st.header("📚 정보교과와 연결된 미래 (진로/학과 심화)") # 헤더 아이콘 유지
st.write("""
...
if checked_count >= 6:
    st.balloons() # 풍선 효과!
    st.write("🎉 와우! 정보교과에 대한 흥미와 적성이 아주 높은 편이네요! 정보교과를 선택하면 정말 재미있게 배우고 큰 성장을 할 수 있을 거예요! 미래 IT 전문가의 꿈을 키워보는 건 어떨까요?")
elif checked_count >= 4:
    st.write("😊 정보교과에 대한 잠재력과 흥미가 충분하네요! 조금만 더 관심을 가지고 노력하면 정보교과에서 좋은 결과를 얻을 수 있을 거예요. 한번 도전해 보는 것을 추천해요!")
elif checked_count >= 2:
    st.write("🙂 아직 정보교과가 낯설 수 있지만, 몇 가지 항목에 해당하는 것을 보니 관심 가질 만한 부분이 있네요! 정보교과 수업을 통해 새로운 재미를 발견할 수도 있어요. 너무 어렵게 생각하지 말고 열린 마음으로 다가가 보세요!")
else:
    st.write("😅 아직 정보교과에 대해 잘 모르거나 크게 흥미를 느끼지 못할 수도 있어요. 하지만 정보교과는 미래 사회에 꼭 필요한 과목이니, 이 가이드를 통해 조금 더 알아보는 건 어떨까요? 생각보다 재미있을 수도 있답니다!")
