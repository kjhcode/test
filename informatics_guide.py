...
# 정보교과 5가지 영역 - 아이콘 변경
info_areas = {
    "컴퓨팅 시스템 💻": "컴퓨터가 어떻게 작동하는지, 하드웨어와 소프트웨어의 기본 원리를 이해해.", # 아이콘 변경
    "데이터 📈": "다양한 데이터를 수집하고 분석해서 의미 있는 정보를 찾아내는 방법을 배워.", # 아이콘 변경
    "알고리즘과 프로그래밍 🧩": "문제를 해결하기 위한 절차(알고리즘)를 설계하고, 컴퓨터 언어(파이썬 등)로 프로그램을 만드는 연습을 해.", # 아이콘 변경
    "인공지능 🤖": "스스로 학습하고 판단하는 인공지능의 기본 개념과 활용 사례를 탐구해.", # 아이콘 유지
    "디지털 문화 🌐": "디지털 세상에서의 올바른 윤리 의식과 시민 역량을 길러." # 아이콘 변경
}
...
st.header("🗣️ 정보교과 선배들의 이야기") # 헤더 아이콘 유지
st.write("""
...
st.header("🗺️ 정보교과와 연결된 미래 (진로/학과 심화)") # 헤더 아이콘 유지
st.write("""
